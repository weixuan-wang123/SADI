...
class LlamaMLP(nn.Module):
    ...
    def forward(self, x):
        if self.config.pretraining_tp > 1:
            ...
            act_up_value = self.act_fn(self.gate_proj(x)) * self.up_proj(x)
            act_up_value = self.act_up(act_up_value)
            down_proj = self.down_proj(act_up_value)
